	...
	private static void assertTestSetIsValid(final UtxoTestSet utxoTestSet) {
		...
		// Instantiate student solution
		 TxHandler txHandler = new TxHandler(utxoTestSet.getUtxoPool());
                ...
                Transaction[] handleTxs = 
                        txHandler.handleTxs( trxsValidation.valid.toArray( new Transaction[trxsValidation.valid.size()]) );
                System.out.println("Valids are "+handleTxs.length);
                
                trxsValidation = utxoTestSet.getValidationLists();
                txHandler = new TxHandler(utxoTestSet.getUtxoPool());
